...
pub struct EguiDx9<T> {
    ui_fn: Box<dyn FnMut(&Context, &mut T) + Send + 'static>,
    ui_state: T,
    hwnd: HWND, // stupid microsoft, why you change type from uint to raw ptr?!
    reactive: bool,
...
impl<T: Send> EguiDx9<T> {
    ...
    pub fn init(
        ...
        hwnd: HWND,
        ui_fn: impl FnMut(&Context, &mut T) + Send + 'static,
        ui_state: T,
        reactive: bool,
    ) -> Self {
        if hwnd.is_invalid() {
